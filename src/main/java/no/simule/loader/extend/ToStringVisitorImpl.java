...
import no.simule.loader.ModelLoader;
...
public class ToStringVisitorImpl<C, O, P, EL, PM, S, COA, SSA, CT>
        extends AbstractVisitor<String, C, O, P, EL, PM, S, COA, SSA, CT> {

    /**
     * Indicates where a required element in the AST was <code>null</code>, so that it is evident in
     * the debugger that something was missing. We don't want just <code>"null"</code> because that
     * would look like the OclVoid literal.
     */
    protected static String NULL_PLACEHOLDER = "\"<null>\""; //$NON-NLS-1$
    private final Environment<?, C, O, P, EL, PM, S, COA, SSA, CT, ?, ?> env;
    // public Stack<String> ModelLoader.expresstionStack = new Stack<>();
     ...
     */
    protected ToStringVisitorImpl(Environment<?, C, O, P, EL, PM, S, COA, SSA, CT, ?, ?> env) {
        this.env = env;
        this.uml = (env == null) ? null : env.getUMLReflection();
    }

    /**
     * Obtains an instance of the <tt>toString()</tt> visitor for the specified environment.
     *
     * @param env an OCL environment
     * @return the corresponding instance
     */
    public static <C, O, P, EL, PM, S, COA, SSA, CT> ToStringVisitorImpl<C, O, P, EL, PM, S, COA, SSA, CT> getInstance(
            Environment<?, C, O, P, EL, PM, S, COA, SSA, CT, ?, ?> env) {

        return new ToStringVisitorImpl<C, O, P, EL, PM, S, COA, SSA, CT>(env);
    }

    /**
     * Obtains an instance of the <tt>toString()</tt> visitor for the specified expression or other
     * typed element.
     *
    ...
    public static <C, O, P, EL, PM, S, COA, SSA, CT> ToStringVisitorImpl<C, O, P, EL, PM, S, COA, SSA, CT> getInstance(
        ...
        return new ToStringVisitorImpl<C, O, P, EL, PM, S, COA, SSA, CT>(
                (Environment<?, C, O, P, EL, PM, S, COA, SSA, CT, ?, ?>) Environment.Registry.INSTANCE
                        .getEnvironmentFor(element));
    }
     ...
     * @return a name, or the null placeholder if the named element or its name be <code>null</code>.
     * i.e., <code>null</code> is never returned
     */
    protected String getName(Object named) {
        String name = (uml == null) ? NULL_PLACEHOLDER : uml.getName(named);
        ;
        return name;
    }
     ...
     *
     * @param named a named element or <code>null</code>
     * @return a qualified name, or the null placeholder if the named element or its name be
     * <code>null</code>. i.e., <code>null</code> is never returned
     */
     ...
     */
    @Override
    protected String handleOperationCallExp(OperationCallExp<C, O> oc, String sourceResult,
        ...
        ModelLoader.expresstionStack.add(OCLTranlateLibraryUtil
                .getOperationName(OCLStandardLibraryUtil.getOperationCode(getName(oper))));

        System.out.println("OppName : " + getName(oper));
    ...
    public String visitVariableExp(VariableExp<C, PM> v) {
        ...
        System.out.println("variable expression: " + result);
        return result;
    }
     ...
     */
    @Override
    protected String handlePropertyCallExp(PropertyCallExp<C, P> pc, String sourceResult,
        ...
        if (sourceResult == null) {
            ...
            if (getName(property).contains("self.")) {
                propertyname = getName(property).substring(5);
            } else {
                propertyname = getName(property);
            }
            ModelLoader.expresstionStack.add(propertyname);
        ...
        StringBuffer result = new StringBuffer(maybeAtPre(pc, sourceResult + "." + getName(property)));//$NON-NLS-1$

        String propertyname = sourceResult + "." + getName(property);
        if (propertyname.contains("self.")) {
            propertyname = propertyname.substring(5);
        }
        ModelLoader.expresstionStack.add(propertyname);
        System.out.println("Property Association  Name : " + propertyname);
    ...
    protected String handleAssociationClassCallExp(AssociationClassCallExp<C, P> ac,
        ...
        StringBuffer result = new StringBuffer(maybeAtPre(ac, sourceResult + "." + name));//$NON-NLS-1$
     ...
     */
    @Override
    protected String handleIfExp(IfExp<C> ifExp, String conditionResult, String thenResult,
                                 String elseResult) {
        StringBuffer result = new StringBuffer();
    ...
    public String visitIntegerLiteralExp(IntegerLiteralExp<C> il) {
        String value =
                (il.getIntegerSymbol() == null) ? NULL_PLACEHOLDER : il.getIntegerSymbol().toString();

        ModelLoader.expresstionStack.add(value);
    ...
    public String visitRealLiteralExp(RealLiteralExp<C> rl) {
        String value = (rl.getRealSymbol() == null) ? NULL_PLACEHOLDER : rl.getRealSymbol().toString();
        ModelLoader.expresstionStack.add(value);
        System.out.println("Value : " + value);
        return value;
    }
    ...
    public String visitStringLiteralExp(StringLiteralExp<C> sl) {
        String value = "'" + ((sl.getStringSymbol() == null) ? NULL_PLACEHOLDER //$NON-NLS-1$
                : sl.getStringSymbol()) + "'";//$NON-NLS-1$
        String value2 = ((sl.getStringSymbol() == null) ? NULL_PLACEHOLDER //$NON-NLS-1$
                : sl.getStringSymbol());//$NON-NLS-1$
        ModelLoader.expresstionStack.add("\"" + value2 + "\"");
        System.out.println("Value : " + "\"" + value + "\"");
        return value;
    ...
    public String visitBooleanLiteralExp(BooleanLiteralExp<C> bl) {
        String value =
                (bl.getBooleanSymbol() == null) ? NULL_PLACEHOLDER : bl.getBooleanSymbol().toString();
        ModelLoader.expresstionStack.add(value);
        System.out.println("Value : " + value);
        return value;
    }
     ...
     */
    @Override
    protected String handleIterateExp(IterateExp<C, PM> callExp, String sourceResult,
     ...
     */
    @Override
    protected String handleIteratorExp(IteratorExp<C, PM> callExp, String sourceResult,
    ...
    @Override
    protected String handleCollectionRange(CollectionRange<C> range, String firstResult,
                                           String lastResult) {
    ...
    @Override
    protected String handleMessageExp(MessageExp<C, COA, SSA> messageExp, String targetResult,
                                      List<String> argumentResults) {
        StringBuffer result = new StringBuffer();
    ...
    protected List<EObject> getConstrainedElements(CT constraint) {
        if (uml == null)
            ...
            return (List<EObject>) uml.getConstrainedElements(constraint);
    }
