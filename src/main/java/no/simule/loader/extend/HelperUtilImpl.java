...
public class HelperUtilImpl {
    ...
    OCLHelperImpl<PK, C, O, P, EL, PM, S, COA, SSA, CT, CLS, E>
    ...
    static String getLogicalLine(String txt) throws Exception {
        ...
        int embeddedCommnetIndex = HelperUtilImpl.NONE;
        String line = reader.readLine();
        while (line != null) {
            line = line.trim();
            if (line.startsWith(HelperUtilImpl.OCL_COMMENT) == false) {
                // safe to search for index of "--" which consists of BMP code points
                embeddedCommnetIndex = line.indexOf(HelperUtilImpl.OCL_COMMENT);
                if (embeddedCommnetIndex != HelperUtilImpl.NONE) {
    ...
    void visit(
        ...
        expression.accept(ToStringVisitorImpl.getInstance(env));
    }
